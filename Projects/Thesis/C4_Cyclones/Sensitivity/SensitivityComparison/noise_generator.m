function y = noise_generator( N , alpha, show_actual_PSE_and_plot )

if nargin<3
    show_actual_PSE_and_plot = false;
end

% N - number of samples to be returned in a row vector
% y - a row vector of noise samples
...
if show_actual_PSE_and_plot
    figure
    loglog(abs(X).^2)
    disp(['alpha = ', num2str(PSE_new(y'))])
end
