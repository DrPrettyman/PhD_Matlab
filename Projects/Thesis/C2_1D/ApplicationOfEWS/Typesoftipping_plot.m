...
% Plots the data contained in the structure "transitions" which is
% produced by running "Typesoftipping.m"
%

%% Load the structure or run the script
%load transitions20
% If there is no structure "transitions", run the scipt "Typesoftipping.m"
...
%for type = 3:3
    ...
    t1 = transitions(type).t1;
    z1 = transitions(type).Zarray(:,1);
    if strcmp(transitions(type).type,'Noise-induced')
        if z1(100) > 0
            z1 = -z1;
        end
    end
    if strcmp(transitions(type).type,'Pitchfork')
        if z1(end) < 0
            z1 = -z1;
        end
    end
    pse_mean1 =...
        transitions(type).mean_pse(...
        transitions(type).pse_windowsize+1:end);
    t1_pse =...
        t1(transitions(type).pse_windowsize+1:end);
    acf_mean1 =...
        transitions(type).mean_acf(...
        transitions(type).acf_windowsize+1:end);
    t1_acf =...
        t1(transitions(type).acf_windowsize+1:end);
    ...
    abc = 'abc';
    annotationmsg = ['(', transitions(type).type, ')'];
