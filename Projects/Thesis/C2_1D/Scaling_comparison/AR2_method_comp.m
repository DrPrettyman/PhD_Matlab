N = 1000;
exponents1 = linspace(0,1,20);
numColours = size(exponents1,2);

numTrials = 3;
exponents2 = linspace(0,1,numTrials);

means = zeros(numColours,4);
errors = zeros(numColours,4);

for k = 1:numColours
    exponent1 = exponents1(k)
    ...
    for trial = 1:numTrials
        exponent2 = exponents2(trial);
        noise = zeros(N,1);
        for i = 3:N
            noise(i) = exponent1*noise(i-1)+...
                exponent2*noise(i-2)+0.1*randn();
        end
