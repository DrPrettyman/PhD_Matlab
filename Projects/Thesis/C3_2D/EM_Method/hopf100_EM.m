...
all_realeigenvals = zeros(300-windowsize, N);
all_imageigenvals = zeros(300-windowsize, N);
all_EOF_ACF1_series = zeros(300-windowsize, N);
all_EOF_DFA_series = zeros(300-windowsize, N);
all_EOF_PS_series = zeros(300-windowsize, N);
for trial = 1:N
    ...
    [tout,rout] = EM_method(f, sigma, x0, tBounds, delta_t);
...
figure
plot(linspace(0,60,numel(rout(:,1))), rout(:,1))
figure
plot(linspace(0,60,numel(rout(:,2))), rout(:,2))
