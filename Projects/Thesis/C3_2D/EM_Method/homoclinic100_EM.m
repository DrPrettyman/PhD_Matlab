...
all_realeigenvals = zeros(200, N);
all_imageigenvals = zeros(size(all_realeigenvals));
all_EOF_ACF1_series = zeros(200-windowsize, N);
all_EOF_DFA_series = zeros(size(all_EOF_ACF1_series));
all_EOF_PS_series = zeros(size(all_EOF_ACF1_series));
...
for trial = 1:N
    ...
    [tout, Y] = EM_method(f, sigma, x0, tBounds, delta_t);
...
time_axis = tout(windowsize+1:end);
figure
