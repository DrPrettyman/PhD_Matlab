...
X1(:,1) = x_0;   
for k = 2:N
    X1(:,k) = Lambda(k)*X1(:,k-1) + S*randn(2,1);
end

% We define the noise term 
S = sigma*(1/sqrt(2))*[[1;1],[1;1]];    
% we run the system for X2
X2 = zeros(2,N);
X2(:,1) = x_0;   
for k = 2:N
    X2(:,k) = Lambda(k)*X2(:,k-1) + S*randn(2,1);
end
