!######################
!  programm to remap the RF99 reference data from the 5min-grid to the model grid, i.e. 10°
...
program remap
  ...
  real :: BIOME_N(36,18)        ! remapped biome distribution in 10°grid (36x18 grid-cells)  (output file)
...
open (9, file='potveg_vecode.txt')                                   ! 10° grid biome output (output file)
...
! remapping from the RF99 grid to 10°grid, the number of grid-cell in the 5min-grid is a multiple of the number of grid-cells in 10°grid, containing 120 x 120 grid cells
do i = 1,18
   do j = 1,36
      coun = 0
      do k = 1,120
         do l = 1,120
            do m = 1,10
               if (BIOME_RF(((j-1)*120+l),((i-1)*120+k)) == REAL(m)) then
                  coun(m) = coun(m) + 1
               end if
            end do
         end do
      end do
! The Biomes in the 10° grid-cells are set to the dominant biome (biome no.t) in the RF99 grid-cells that are located in the coarser 10° grid-cells
       t = maxloc(coun, DIM=1)
       BIOME_N(j,i) = REAL(t)
   end do
end do
...
! write output files
do i = 1,18
   write(9,'(36(f5.1,1x))')(BIOME_N(j,i),j=1,36)
end do
do i = 1,2160
